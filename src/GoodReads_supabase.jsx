...
/* 브라우저 저장소 (시크릿 모드 등에서 막혀도 앱이 죽지 않게 감싸둠) */
function safeGet(k) {
  try {
...
export default function GoodReads() {
  ...
  useEffect(() => {
    ...
    const stored = safeGet(ME_KEY);
    if (stored) setMe(stored);
    else setAskName(true);
    if (safeGet(PASS_KEY) === PASSWORD) setUnlocked(true);
    return () => {
      live = false;
    };
  }, []);
  ...
  return (
    <div style={S.shell}>
      ...
      {askName && <NamePrompt onDone={(n) => { setMe(n); safeSet(ME_KEY, n); setAskName(false); }} />}
...
function PasswordGate({ onUnlock }) {
  ...
  return (
    <div style={S.gateWrap}>
      <img src={LOGO} alt="훌라 로고" style={S.gateLogo} />
      <h1 style={S.gateTitle}>훌라</h1>
      <p style={S.gateDesc}>모임 비밀번호를 입력해주세요</p>

      <input
        autoFocus
        type="password"
        style={{ ...S.gateInput, ...(wrong ? S.gateInputWrong : {}) }}
        placeholder="비밀번호"
        value={v}
      ...
      {wrong && <p style={S.gateError}>비밀번호가 맞지 않아요</p>}

      <button
        style={{ ...S.primaryBtn, width: "100%", marginTop: 12, opacity: v.trim() ? 1 : 0.5 }}
        disabled={!v.trim()}
        onClick={tryUnlock}
      >
        들어가기
      ...
      <p style={S.gateHint}>한 번 입력하면 이 기기에서는 계속 기억해요</p>
    </div>
  );
}
...
const S = {
  ...
  gateLogo: { width: 76, height: "auto", marginBottom: 18 },
  gateTitle: { margin: 0, fontSize: 28, fontWeight: 800, letterSpacing: "-0.03em" },
  gateDesc: { margin: "8px 0 22px", fontSize: 14, color: C.textSec },
  gateInput: {
    width: "100%", border: `0.5px solid ${C.borderStrong}`, borderRadius: 11,
    padding: "13px 15px", fontSize: 16, color: C.ink, background: C.surface,
    fontFamily: "inherit", textAlign: "center", boxSizing: "border-box",
  },
  gateInputWrong: { borderColor: "#E24B4A", background: "#FFF6F6" },
  gateError: { margin: "8px 0 0", fontSize: 13, color: "#E24B4A", fontWeight: 500 },
  gateHint: { margin: "16px 0 0", fontSize: 12, color: C.textTer },
