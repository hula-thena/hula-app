...
const CATEGORIES = [
  { id: "essay", label: "에세이", color: "#7F77DD" },
  { id: "book", label: "책", color: "#1D9E75" },
  { id: "news", label: "시사", color: "#378ADD" },
  { id: "quote", label: "글귀", color: "#D85A30" },
  { id: "life", label: "일상", color: "#D4537E" },
    ...
    <div style={S.overlay}>
      <div style={S.modal}>
        ...
        <p style={{ color: C.textSec, fontSize: 14, margin: "0 0 16px", lineHeight: 1.6 }}>
          모임에서 쓸 이름을 알려주세요. 글과 댓글에 이 이름으로 표시돼요.
        </p>
        <input
          autoFocus
          style={S.input}
...
/*  Feed                                                               */
/* ------------------------------------------------------------------ */
function Feed({ posts, me, onLike, onComment, onTag, onDelete }) {
  if (posts.length === 0) {
    return (
      <div style={S.empty}>
...
function PostCard({ post, me, onLike, onComment, onTag, onDelete }) {
  ...
  return (
    <article style={S.card}>
      <div style={S.cardHead}>
        ...
        {mine && (
          <button style={S.iconBtn} onClick={() => onDelete(post.id)} aria-label="삭제" title="삭제">
            <Trash2 size={15} />
          </button>
        )}
      </div>
...
/*  Calendar view                                                      */
/* ------------------------------------------------------------------ */
function CalendarView({ posts, month, setMonth, openPost, setOpenPost, me, onLike, onComment, onTag, onDelete }) {
  const { y, m } = month;
...
const C = {
  ...
  borderStrong: "rgba(44,44,42,0.18)",
  accent: "#7F77DD",
};
...
const S = {
  ...
  brand: { margin: 0, fontSize: 26, fontWeight: 700, letterSpacing: "-0.02em", fontFamily: "'Nanum Myeongjo', 'Pretendard', serif" },
  ...
  linkBox: {
    ...
    background: "#F4F1EA", border: `0.5px solid ${C.border}`, borderRadius: 10,
  ...
  commentInput: { flex: 1, height: 38, border: `0.5px solid ${C.border}`, borderRadius: 10, padding: "0 12px", fontSize: 14, background: "#FAFAF8" },
  ...
  modalTitle: { margin: 0, fontSize: 19, fontWeight: 700, letterSpacing: "-0.01em", fontFamily: "'Nanum Myeongjo', serif" },
  ...
  typeToggle: { display: "flex", gap: 6, marginBottom: 14, background: "#F4F1EA", padding: 4, borderRadius: 12 },
  ...
  input: {
    ...
    fontSize: 15, color: C.ink, background: "#FAFAF8", fontFamily: "inherit",
  },
  ...
  calTitle: { fontSize: 17, fontWeight: 700, fontFamily: "'Nanum Myeongjo', serif" },
